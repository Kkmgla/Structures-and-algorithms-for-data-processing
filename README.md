...
### Target
Gaining skills in using bitwise operations in algorithms.

### Exercise
Develop a program that demonstrates the implementation of various exercises. The results of the exercises and input values should be displayed on the monitor in both decimal and binary forms.

### Exercise Requirements
1. Define a variable of integer type and assign a value to it using a constant in hexadecimal notation.
2. Develop a function that will set the bits of the initial value of a variable to 1 using the appropriate mask and bitwise operation.
...
## SIAOD2 (EN)
### Subject
External data structures: text and binary files.

### Target
Gain skills in using C++ file streams (or C language files) to manage text and binary files.

### Exercise 1
Develop a program to manage a text file.
1. Implementation of I/O based on C++ file streams: ofstream, ifstream.
2. The name of the physical file is entered by the user and passed to the processing functions via a parameter.
3. Develop functions to perform operations on a text file:
   - Creating an ASCII text file containing decimal numbers, several numbers per line.
   - Outputting the contents of a text file.
   - Adding a new line to the end of the file.
   - Reading the value of a number, indicating its serial number in the file, and returning its value.
   - Determining the number of numbers in the file.
   - Implementing additional operations defined by options.
...
### Exercise 2
Develop a binary file management program with fixed-length records. General requirements: the file consists of records of a certain structure, according to the option. Records have a key that is unique within the file.
1. Develop a binary file recording structure according to the assignment option.
2. Prepare test data in a text file with ASCII encoding, in accordance with the structure of the variant record. When opening a file, check its existence and opening.
3. The file name is entered by the user.
4. When opening a file, provide control over the existence and opening of the file.
5. When using the mechanism of direct access to a file record, check the presence of a record with a given number in the file.
6. Develop functions to perform operations:
   - Converting test data from a text file to a binary file.
   - Converting data from a binary file to a text file.
   - Outputting all entries of a binary file.
   - Accessing a record by its serial number in the file, using the mechanism of direct access to a record in a binary file.
   - Deleting a record with a given key value by replacing it with the last record.
   - Manipulating entries in a binary file according to additional operations defined in the variant.
...
## Subject
Using a hash table to look up data in a binary file with fixed-length entries.

### Target
Gain skills in developing hash tables and using them when searching for data in other data structures (files).
...
## SIAOD4 (EN)
### Subject
Nonlinear data structures. Binary tree.

### Target
Obtaining skills in developing and implementing operations on the binary tree data structure.
...
#### Meaning of the Information Part
Real number
...
## SIAOD5 (EN)
### Subject
Balanced search trees (BSTs) and their use for searching data in a file.

### Target
Gain skills in the development and implementation of algorithms for managing a binary search tree and balanced binary search trees (ABL trees); gain skills in using file streams for direct access to file data; Gain skills in using a balanced search tree to directly access file records.
...
### Balanced Type Search Tree (STD)
B-Tree

### Record Structure
Company employees: number (key), full name, fact of higher education (bool)

## SIAOD6 (EN)
### Subject
Basic algorithms for working with graphs.

### Target
Obtaining practical skills in performing operations on graph data structures.
...
## SIAOD7 (EN)
### Subject
Encoding and compression of data using lossless methods.

### Target
Obtaining practical skills and knowledge to perform data compression using the methods in question.
...
## SIAOD8 (EN)
### Subject
Algorithmic strategies or methods for developing algorithms. Overkill and methods for reducing it.

### Target
Gain skills in using methods to reduce the number of searches in problems that can only be solved by searching through all possible solution options.
...
### Цель
Получить навыки применения файловых потоков языка С++ (или файлов языка Си) по управлению текстовым и двоичным файлами.
...
4. Управление хеш-таблицей
    ...
    4.6. Разработать операции: вставить ключ в таблицу, удалить ключ из таблицы, найти ключ в таблице, рехешировать таблицу. Каждую операцию тестировать по мере ее реализации.
